...
function! <SID>GetDoxygenStyleDoc(fun_str, fun_info, leading_blank)

	let doc=""
	let idx=1
"	let doc = a:leading_blank."//{{{".a:fun_str."\n"
...
function! <SID>GetDoxFileHeader(leading_blank)
  ...
  let doc = doc. a:leading_blank."/***************************************************************************\n"
	let doc = doc. a:leading_blank." * \n"
  let doc = doc. a:leading_blank." * Copyright (c) ".<SID>GetYear()."Brightcells, Inc. All Rights Reserved\n"
...
function! <SID>GetDoxStruct(leading_blank)
	let doc = ""
	let doc = doc.a:leading_blank."/**\n"
...
function! <SID>GenDoc()
	...
	let doc = <SID>GetDoxygenStyleDoc(fun_str, fun_info, leading_blank)
	"echo "doc \n".expand(doc)
...
function! <SID>GetDoxFH(type)
	...
	elseif (a:type == 2)
		let doc = <SID>GetDoxStruct(leading_blank)
		if (strlen(matchstr(first_line, '\S'))==0)
			return
		endif
	elseif (a:type == 3)
...
autocmd FileType c,cpp,java,sh,awk,vim,sed,perl,python nnoremap <silent> <Leader>a :call <SID>GetDoxFH(0)<CR>
autocmd FileType c,cpp,java,sh,awk,vim,sed,perl,python nnoremap <silent> <Leader>s :call <SID>GetDoxFH(1)<CR>
autocmd FileType c,cpp,java,sh,awk,vim,sed,perl,python nnoremap <silent> <Leader>d :call <SID>GetDoxFH(2)<CR>
autocmd FileType c,cpp,java,sh,awk,vim,sed,perl,python nnoremap <silent> <Leader>f :call <SID>GetDoxFH(3)<CR>
autocmd BufNewFile *.h,*.cpp,*.c,*.cc,*.java,*.py,*.pl,*.php  :call <SID>InsertFormat()
